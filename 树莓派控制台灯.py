...
说明：此代码使用预训练的EfficientNet-B0模型（.tflite格式）
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("emotion_light.log"),
...
class EfficientNetEmotionDetector:
    """使用TFLite运行预训练的EfficientNet-B0进行情绪识别"""
    
    def __init__(self, model_path):
        """初始化TFLite解释器并加载预训练模型"""
        try:
            ...
            logger.debug(f"模型输入详情: {self.input_details[0]['dtype']}, {self.input_details[0]['shape']}")
            logger.debug(f"模型输出详情: {self.output_details[0]['dtype']}, {self.output_details[0]['shape']}")
    ...
    def preprocess_image(self, image):
        ...
        注意：此处的预处理必须与模型训练时的预处理完全一致！
        EfficientNet-B0标准预处理：
        1. 调整大小到224x224
        2. 应用EfficientNet特定的归一化（ImageNet均值/标准差）
        """
        try:
            # 调整大小到模型输入尺寸
            ...
            # EfficientNet标准预处理（ImageNet均值/标准差）
            # 这些值必须与模型训练时使用的值一致！
...
class WiFiLampController:
    ...
    def _connect(self):
        ...
        try:
            ...
            # 初始状态：开启并设置为柔和光
            self.lamp.on()
            self.lamp.set_rgb(255, 255, 255)
            self.lamp.set_brightness(128)  # 50%亮度
    ...
    def set_emotion_light(self, emotion, confidence=1.0):
        """
        根据情绪设置灯光
        
        参数:
            emotion: 情绪标签
            confidence: 置信度 (0.0-1.0)，用于调整灯光效果强度
        """
        ...
        try:
            # 情绪到灯光参数的映射（根据你的要求）
            # 格式: {'brightness': 亮度百分比(0-100), 'rgb': (R, G, B)}
            light_config = {
                'happy':     {'brightness': 85, 'rgb': (255, 200, 100)},    # 开心：暖橙色
                'neutral':   {'brightness': 65, 'rgb': (220, 230, 255)},    # 平静：淡蓝色
                'sad':       {'brightness': 45, 'rgb': (150, 180, 255)},    # 低落：冷蓝色
                'angry':     {'brightness': 55, 'rgb': (255, 100, 100)},    # 烦躁：浅红色
                'surprise':  {'brightness': 70, 'rgb': (255, 255, 150)},    # 惊讶：淡黄色
                'fear':      {'brightness': 40, 'rgb': (100, 100, 200)},    # 恐惧：蓝色
                'disgust':   {'brightness': 50, 'rgb': (150, 200, 100)},    # 厌恶：黄绿色
            }
            
            # 获取配置，如果情绪未定义则使用中性光
            if emotion in light_config:
                config = light_config[emotion]
            else:
                logger.warning(f"未定义的情绪 '{emotion}'，使用中性光")
                config = light_config['neutral']
    ...
    def set_default_light(self):
        ...
        if self.is_connected:
            try:
                self.lamp.set_rgb(255, 220, 180)  # 暖白色
                self.lamp.set_brightness(76)      # 30%亮度
                logger.info("已设置默认柔和灯光")
            except Exception as e:
                logger.error(f"设置默认灯光失败: {e}")
...
class CameraManager:
    ...
    def capture_image(self):
        ...
        try:
            ...
            logger.debug(f"图像捕获成功，尺寸: {captured_frame.shape}")
...
def emotion_detection_task():
    ...
    # logger.debug(f"图像已保存: capture_{timestamp}.jpg")
    
    # 释放摄像头（长时间占用摄像头可能有问题）
    camera.release()
    
    # 可选：进行人脸检测（可提高准确性）
    has_face, faces = detect_faces(frame)
    
    if not has_face:
        logger.warning("未检测到人脸，跳过情绪识别")
        # 可以选择设置默认灯光或保持当前状态
        lamp_controller.set_default_light()
        return
    ...
    logger.info(f"任务完成，耗时: {task_duration:.1f}秒")
    logger.info("=" * 50)
...
def main():
    ...
    print("  pip3 install tflite-runtime python-miio schedule")
    print("=" * 60)
    
    global detector, lamp_controller
    ...
    try:
        ...
        if not lamp_controller.is_connected:
            logger.warning("台灯连接失败，程序将继续运行但无法控制灯光")
        
        # 设置定时任务
        logger.info(f"设置定时任务，每{CHECK_INTERVAL}分钟执行一次...")
        schedule.every(CHECK_INTERVAL).minutes.do(emotion_detection_task)
        ...
        print("\n程序运行中...")
        print("情绪识别日志将显示在上方并保存到 emotion_light.log")
        print("-" * 60)
        
        # 主循环：每分钟检查一次是否有任务需要执行
        while True:
        ...
        # 设置柔和的默认灯光
        if 'lamp_controller' in globals() and lamp_controller:
            lamp_controller.set_default_light()
